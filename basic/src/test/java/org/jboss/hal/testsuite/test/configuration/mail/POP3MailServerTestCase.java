...
public class POP3MailServerTestCase extends MailServerTestCaseAbstract {
    private static final String POP3 = "pop3";

    private static final Address POP3_SERVER_ADDRESS = MAIL_SESSION_ADDRESS.and("server", POP3);
    ...
    public static void cleanUp() throws IOException, OperationException, InterruptedException, TimeoutException, CommandFailedException {
        try {
            operations.removeIfExists(POP3_SERVER_ADDRESS);
    ...
    /**
     * @tpTestDetails Try to create POP3 mail server in Web Console's mail subsystem configuration.
    ...
    public void createMailServer() throws Exception {
        ...
        try {
            ...
            new ResourceVerifier(POP3_SERVER_ADDRESS, client)
                    .verifyExists()
    ...
    /**
     * @tpTestDetails Create POP3 mail server instance in model and try to remove it in Web Console's mail subsystem configuration
    ...
    public void removeMailServer() throws Exception {
        try {
            createMailServerInstanceInModel(POP3_SERVER_ADDRESS, SOCKET_BINDING);
            page.viewMailSession(MAIL_SESSION_NAME);
            page.getResourceManager().removeResource(POP3.toUpperCase()).confirmAndDismissReloadRequiredMessage();
            new ResourceVerifier(POP3_SERVER_ADDRESS, client).verifyDoesNotExist();
        } finally {
            operations.removeIfExists(POP3_SERVER_ADDRESS);
            administration.reloadIfRequired();
        }
    ...
    /**
     * @tpTestDetails Create POP3 mail server instance in model and try to set its credential reference attribute
    ...
    public void setCredentialReferenceToClearText() throws Exception {
        try {
            createMailServerInstanceInModel(POP3_SERVER_ADDRESS, SOCKET_BINDING);
            operations.undefineAttribute(POP3_SERVER_ADDRESS, PASSWORD_ATTRIBUTE);
            page.viewMailSession(MAIL_SESSION_NAME);
            page.getResourceManager().selectByName(POP3);
            page.switchToCredentialReferenceTab();
            new ElytronIntegrationChecker.Builder(client)
                    .configFragment(page.getConfigFragment())
                    .address(POP3_SERVER_ADDRESS)
                    .build()
    ...
    /**
     * @tpTestDetails Create POP3 mail server instance in model and try to set its username attribute
    ...
    public void editUsername() throws Exception {
        try {
            ...
            new ConfigChecker.Builder(client, POP3_SERVER_ADDRESS)
                    .configFragment(page.getConfigFragment())
    ...
    /**
     * @tpTestDetails Create POP3 mail server instance in model and try to set its password attribute
     * in Web Console's mail subsystem configuration
     * Validate attribute in model.
     */
    @Test
    ...
    /**
     * @tpTestDetails Create POP3 mail server instance in model and try to toggle its ssl attribute
     * in Web Console's mail subsystem configuration
     * Validate attribute in model.
     */
    @Test
    public void toggleUseSSL() throws Exception {
        try {
            ...
            page.viewMailSession(MAIL_SESSION_NAME);
            page.getResourceManager().selectByName(POP3);
            final ModelNodeResult nodeResult = operations.readAttribute(POP3_SERVER_ADDRESS, SSL_ATTRIBUTE);
    ...
    /**
     * @tpTestDetails Create POP3 mail server instance in model and try to toggle its tls attribute
     * in Web Console's mail subsystem configuration
     * Validate attribute in model.
     */
    @Test
    public void toggleUseTLS() throws Exception {
        try {
            ...
            page.viewMailSession(MAIL_SESSION_NAME);
            page.getResourceManager().selectByName(POP3);
            final ModelNodeResult nodeResult = operations.readAttribute(POP3_SERVER_ADDRESS, TLS_ATTRIBUTE);
            nodeResult.assertSuccess();
    ...
    /**
     * @tpTestDetails Create POP3 mail server instance in model and try to edit its socked binding attribute
     * in Web Console's mail subsystem configuration
     * Validate attribute in model.
     */
    @Test
    ...
    /**
     * @tpTestDetails Create POP3 mail server instance in model and try to create another one
     * in Web Console's mail subsystem configuration
